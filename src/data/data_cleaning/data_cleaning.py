...
class Data_Cleaning :
    ...
    def impute_missing_values(self, df : pd.DataFrame, target : str, train :bool) :
        ...
        if train :
            ...
            self.df_train_na_col = (pd.DataFrame(((df_train.isna().sum(axis=0))/len(df_train))*100)).rename(columns={0: 'Sum_NaN_%'})
            #drop col :
            df_train = df_train.drop(columns=list((self.df_train_na_col[self.df_train_na_col["Sum_NaN_%"]>50]).index))
            ...
            #drop row :
            df_train = df_train.drop(index=list((df_train_na_row[df_train_na_row["Sum_NaN_%"]>60]).index))
            #reset_index :
            ...
            self.df_train_grouby_3var = df_train.groupby(["PASS_GRACE_IND_M1", "PASS_GRACE_IND_M2", "PASS_GRACE_IND_M3"])
            self.df_train_grouby_2var = df_train.groupby(["PASS_GRACE_IND_M1", "PASS_GRACE_IND_M2"])
            self.df_train_grouby_1var = df_train.groupby(["PASS_GRACE_IND_M1"])
            #*************************************************************************************
            ...
            for i in (df_train_na_diff_0.index) : # or replace (df_train_na_diff_0.index) by list_median_mode_imputation
                if is_numeric_dtype(df_train[i]) and i != target :
                    df_train[i] = self.df_train_grouby_3var[i].fillna(df_train[i].median()) 
                    df_train[i] = self.df_train_grouby_2var[i].fillna(df_train[i].median())
                    df_train[i] = self.df_train_grouby_1var[i].fillna(df_train[i].median())
            ...
            for i in (df_train_na_diff_0.index) :  # or replace (df_train_na_diff_0.index) by list_median_mode_imputation
                if is_object_dtype(df_train[i]) and i != target :
                    df_train[i] = self.df_train_grouby_3var[i].fillna(df_train[i].mode()[0]) ##df_train.groupby(["PASS_GRACE_IND_M1", "PASS_GRACE_IND_M2", "PASS_AFTERGRACE_IND_M2"])[i].fillna(df_train[i].mode())
                    df_train[i] = self.df_train_grouby_2var[i].fillna(df_train[i].mode()[0])
                    df_train[i] = self.df_train_grouby_1var[i].fillna(df_train[i].mode()[0])
            ...
            #If there is 50%-60% or more NaN, drop the column.
            #drop col :
            df_val.drop(columns=list((self.df_train_na_col[self.df_train_na_col["Sum_NaN_%"]>50]).index), inplace=True)
            #Count the number of NaN in rows :
            #If there is 60-70% or more NaN, drop the row.
            ...
            def imputate_missing_3val_num(df) :
                if pd.isna(df[i]) :
                    return (self.df_train_grouby_3var.get_group(("PASS_GRACE_IND_M1"==df["PASS_GRACE_IND_M1"], "PASS_GRACE_IND_M2"==df["PASS_GRACE_IND_M2"], "PASS_GRACE_IND_M3"==df["PASS_GRACE_IND_M3"]))[[elem for elem in df_val.columns if is_numeric_dtype(df_val[elem])]].agg("median"))[i]
                else :
                    return df[i]

            def imputate_missing_2val_num(df) :
                if pd.isna(df[i]) :
                    return (self.df_train_grouby_2var.get_group(("PASS_GRACE_IND_M1"==df["PASS_GRACE_IND_M1"], "PASS_GRACE_IND_M2"==df["PASS_GRACE_IND_M2"]))[[elem for elem in df_val.columns if is_numeric_dtype(df_val[elem])]].agg("median"))[i]
                else :
                    return df[i]

            def imputate_missing_1val_num(df) :
                if pd.isna(df[i]) :
                    return (self.df_train_grouby_1var.get_group(("PASS_GRACE_IND_M1"==df["PASS_GRACE_IND_M1"]))[[elem for elem in df_val.columns if is_numeric_dtype(df_val[elem])]].agg("median"))[i]
                else :
                    return df[i]
            ...
            def imputate_missing_1val_cat(df) :
                if pd.isna(df[i]) :
                    return (self.df_train_grouby_1var.get_group(("PASS_GRACE_IND_M1"==df["PASS_GRACE_IND_M1"]))[[elem for elem in df_val.columns if is_object_dtype(df_val[elem])]].agg("mode"))[i][0]
                else :
                    return df[i]
